...
<div class="container px-0 films-admin">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
        <div>
            <h2 class="mb-1">Ajouter un média</h2>
            <p class="text-muted mb-0">Uploadez une image ou vidéo, ou ajoutez un lien externe.</p>
        </div>
    ...
    <div class="card border-0 shadow-sm" style="border-radius:14px;">
        <div class="card-body p-4 p-md-5">
            <form method="POST" action="{{ route('admin.galeries.store') }}" enctype="multipart/form-data" class="row g-3">
                @csrf
                    ...
                    <select name="film_id" id="film_id" class="form-select @error('film_id') is-invalid @enderror">
                        ...
                        @foreach($films as $film)
                            <option value="{{ $film->id }}"
                                {{ (old('film_id', $selectedFilm) == $film->id) ? 'selected' : '' }}>
                                {{ $film->titre }} ({{ $film->annee_production }})
                            </option>
                        @endforeach
                    </select>
                    <div class="form-text">Laissez vide si le média est lié au festival en général.</div>
                ...
                {{-- Upload fichier --}}
                <div class="col-12 col-md-8">
                    <label for="fichier" class="form-label">Fichier à uploader</label>
                    <input type="file" name="fichier" id="fichier" class="form-control @error('fichier') is-invalid @enderror"
                           accept="image/*,video/*">
                    <div class="form-text">Image : JPEG, PNG, WebP — max 4 Mo. Vidéo : MP4, AVI, MOV — max 200 Mo.</div>
                ...
                <div class="col-12">
                    ...
                    <input type="url" name="lien" id="lien" class="form-control @error('lien') is-invalid @enderror"
                           value="{{ old('lien') }}" placeholder="https://youtube.com/watch?v=...">
                ...
                <div class="col-12 d-flex gap-2 justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-success">Enregistrer le média</button>
                </div>
            </form>
        </div>
    </div>
